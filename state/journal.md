...
- **2026-07-22 (Day 4)** — v2.1 tune: lowered ENTRY.minCh6hPct 8%→6%. Diagnostic across the full liquid Base universe (23 aged/liquid pairs) showed the market is genuinely flat — strongest 6h trend was KEYCAT +4.1% (and its volume $12k/6h was far below the $150k floor). v2 correctly took zero trades (forcing entries into chop is exactly what lost v1 money). 6% still requires a real sustained uptrend; anti-spike guards (maxCh1hPct 5, +24h required, 6h-ranking) unchanged so v1's top-buying flaw stays fixed. Even at 6% nothing qualifies this cycle — confirms no trade was manufactured. Equity $120.70.
- **2026-07-21 late (Day 4)** — v2.2 tune: lowered volume floors (1h $30k→$18k, 6h $150k→$90k). Diagnostic root cause of the long no-trade streak: the ONLY tokens trending on Base right now (KEYCAT +10%/24h +6%/6h, SPX +5%/6h) pass every price/trend gate but are volume ghost towns ($6.7k/1h) — the market is in a low-volume regime where trend and volume are anti-correlated. Deep-liquidity floor ($400k) is the real exit-safety guard, so the volume floor was over-conservative; trimmed it but kept it high enough that KEYCAT-thin names ($24k/6h) still don't qualify (confirmed: no trade this cycle). Not forcing a fill to beat the deadline. Equity $120.60.
